...
namespace caco_alch {
	...
	struct OutputFormat {
		...
		OutputFormat(const opt::ParamsAPI2& args, const std::optional<file::ini::INI>& ini = std::nullopt) :
			...
			_indent{ str::stoui(args.typegetv<opt::Option>("indent"s).value_or([&ini]() -> std::string {
			if (ini.has_value())
				if (const auto value{ ini.value().getv("format", "indent") }; !value.has_value())
					return value.value();
			return "2";
				}())) },
			_precision{ str::stoui(args.typegetv<opt::Option>("precision").value_or([&ini]() -> std::string {
					if (ini.has_value())
						if (const auto value{ ini.value().getv("format", "precision") }; !value.has_value())
							return value.value();
					return "2";
				}())) }
					//	ColorAPI{ ini.has_value() ? loadColorConfig(ini.value()) : ColorAPI{ DefaultObjects._defaultColorAPI } }
				{}
